from machine import Pin, UART
import time, utime
import dht
import network
import urequests

def toCoordinates(toParse):
    RawAsFloat = float(ToParse)
    firstdigits = int(RawAsFloat/100)
    nexttwodigits = RawAsFloat - float(firstdigits*100)
    
    Converted = float(firstdigits + nexttwodigits/60.0)
    Converted = '{0:.6f}'.format(Converted)
    return str(Converted)

def getGPS(gpsModule):
    global FIX_STATUS, TIMEOUT, latitude, longitude, satellites, GPStime
    
    timeout = time.time() + 8
    while True:
        print(gpsModule.readline())
        buff = str(gpsModule.readline())
        parts = buff.split(',')
    
        if (parts[0] == "b'$GPGGA" and len(parts) == 15):
            if(parts[1] and parts[2] and parts[3] and parts[4] and parts[5] and parts[6] and parts[7]):
                print(buff)
                
                latitude = convertToDegree(parts[2])
                if (parts[3] == 'S'):
                    latitude = -latitude
                longitude = convertToDegree(parts[4])
                if (parts[5] == 'W'):
                    longitude = -longitude
                satellites = parts[7]
                GPStime = parts[1][0:2] + ":" + parts[1][2:4] + ":" + parts[1][4:6]
                FIX_STATUS = True
                break
                
        if (time.time() > timeout):
            TIMEOUT = True
            break
        utime.sleep_ms(500)
        
gpsModule = UART(1, baudrate=9600, tx=Pin(4), rx=Pin(5))
print(gpsModule)

buff = bytearray(255)

TIMEOUT = False
FIX_STATUS = False

latitude = ""
longitude = ""
satellites = ""
GPStime = ""

wlan = network.WLAN(network.STA_IF)
wlan.active(True)
wlan.connect("IZZI-7FB4","9CC8FC877FB4")

sensor = dht.DHT11(Pin(13))

while True:
    time.sleep(30)     # polling each half minute
    sensor.measure()
    print("Temperature: "+str(round(sensor.temperature(), 1))+" °C")
    print("Humidity: "+str(round(sensor.humidity(), 1))+"%")
    
    temp_req = "http://192.168.0.2:8080/api/devices/users/toribio/temperature/" + str(round(sensor.temperature(), 1))
    hum_req = "http://192.168.0.2:8080/api/devices/users/toribio/humidity/" + str(round(sensor.humidity(), 1))
    
    r = urequests.post(temp_req)
    r.close()
    time.sleep(2) #to avoid memno error
    r = urequests.post(hum_req)
    r.close()
    time.sleep(2)
    
    getGPS(gpsModule)

    if(FIX_STATUS == True):
        location_req = "http://192.168.0.2:8080/api/devices/users/toribio/location/" + str(latitude) + "/" + str(longitude)
        r = urequests.post(location_req)
        r.close()
        
        print("----------------------")
        print(" ")
        print("Latitude: "+latitude)
        print("Longitude: "+longitude)
        print("Satellites: " +satellites)
        print("Time: "+GPStime)
        print("----------------------")
        
        
        FIX_STATUS = False
        
    if(TIMEOUT == True):
        print("No GPS data is found.")
        TIMEOUT = False
    
